...
def create_enum(n):
    ...
    defs = [
        (f"{depth.name}C{i}", make_type(depth, i))
        for depth in Depth
...
class Mat:
    def __init__(self, m, size, flags):
        ...
        steps = np.asarray(
            [int(m["step"]["p"][i]) for i in range(size.dims())], dtype=np.int64
        )
        self.view = np.lib.stride_tricks.as_strided(
            self.mat[start:], shape=size.to_numpy(), strides=steps
        )
