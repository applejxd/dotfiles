...
| --- | --- | --- | --- | --- | --- |
| **LLM classifier（`ask` → `allow`）** | 機構は実証済み（plugin が effect を書き換えられる）。専用の安価モデルなら 1 回 $0.0006 / 1.1 秒で、**当初の「トークンが嵩む」という反対理由は弱まった** | **自動実行では `allow` と `ask` が等価**なので自動化率は 1% も上がらない。効果は対話時の確認回数だけ。誤りの代償が非対称で、誤許可は**誰も見ないまま危険な操作が走る** | 偽許可率（未測定）。インジェクション耐性（1 例のみ） | **見送り** | — |
| **LLM classifier（`ask` → `deny`）** | **deny だけが自動実行で意味を持つ**。静的パターンがクォート・変数・符号化で破られる穴を意味で補える。誤検知は「余計に止まる」＝安全側 | 遅延が全コマンドに乗る（実履歴 466 件で +1.1 秒 × 件数）。入力（コマンド文字列）は攻撃者に操作されうる | 偽許可率・偽拒否率。「不安定」の具体的な中身 | **未評価** | 実履歴 466 件にラベル付けして偽許可率を測る。**1 件でも出たら却下** |
...
### classifier を再検討した経緯（2026-09-22）

この案件は「LLM classifier への委譲を廃止する」ことから始まった。
[CHG-0003](0003-ask-command-description.md) で安価モデルによる説明生成を
実装した結果、**当時の反対理由の一方（コスト）が測定値で弱まった**ため、
承認・拒否の 2 方向に分けて評価し直した。

- **承認方向は見送り。** 自動実行で `allow` と `ask` が等価である以上、
  得るものが無いのに誤許可の危険だけを負う
- **拒否方向は未評価のまま残す。** 自動実行で効く唯一の方向で、
  段階 2 の目的（穴塞ぎ）とも整合する

**どちらも「不安定」という当初の反対理由は未測定のまま。** 進めるなら
評価基盤（ラベル付き履歴と偽許可率の測定）が先で、実装はその後。
