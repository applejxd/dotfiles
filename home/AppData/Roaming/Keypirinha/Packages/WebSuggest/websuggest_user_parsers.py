...
def default_parser(plugin, provider, response):
    try:
        ...
        plugin.warn("Failed to parse response from provider {}.".format(provider.label))
        traceback.print_exc()
        return []
...
def my_parser(plugin, provider, response: bytes):
    try:
        ...
        url: str = (
            "http://public.dejizo.jp/NetDicV09.asmx/GetDicItemLite?Dic=EJdict&Loc=&Prof=XHTML&Item="
            + id_txt
        )
