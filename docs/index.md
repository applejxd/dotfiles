...
| --- | --- | --- | --- |
| [CHG-0001](change/0001-compaction-context-handover.md) | In progress | **対象を OpenCode V2 へ絞った**。専用の口があり、両 CLI で要った注入遅れの回避が不要 | `compaction` フックの発火条件と `ctx.storage` のスコープ |
| [CHG-0002](change/0002-opencode-ask-by-default.md) | In progress | 段階 3 まで配備済み。**CHG-0004 完了で保留を解き、計画を 3 分割**（4a 実施 / 4b 見送り / 5 は第一サポート決定待ち） | 段階 4a（読み取り専用コマンドの静的 allow） |
