...
### hook が判定を返せないと素通りになる

6 巡目で判明した最も重要な点。CLI は hook がクラッシュしたりタイムアウトすると
**判定なしとして扱い、コマンドをそのまま実行する**。つまり以下は全ガードの回避になる。

- 不正なペイロード (`[]` / `null` / `command` が配列や数値) で hook を落とす
- 5 万文字の引数や 2000 個のセグメントで処理時間を引き延ばす

対策として `main()` に型検証・自前タイムアウト (10 秒)・チェック単位の例外捕捉を入れ、
