...
| 2 | 普段の作業を 1 周通す（認証・skills・MCP・編集・再開） | **進行中**（2026-09-25 開始。Pi と併用して常用する） |
| 2.5 | 使ってみて決まった設定を chezmoi へ戻す（back import、または生成） | 未着手（段 2 がひと段落してから） |
...
**「丸ごと管理しない」という既存の決定と衝突させない。** `config.yml` には
認証・セッション・自動生成 memory が混ざるので、`chezmoi add` で丸ごと取ると
秘密情報ごと取り込む。回収するときは次を仕分ける。

| 種類 | 扱い |
| --- | --- |
| 意図して変えた設定 | 回収対象。`run_onchange_after_` の種まきへ足すか、真偽値なら控えの鍵を増やす |
| 認証・セッション・memory | 回収しない |
| 既定値のまま | 回収しない（明示すると既定の変化に追随できなくなる） |

差分の取り方は `omp config list` の出力を導入直後と比べるのが素直
（`config.yml` の生テキスト差分だと自動生成分が混ざる）。
...
- **`enabledProviders: [claude]` の効果を実機で確認できていない。**
  ...
  **段 2 に着手したので、常用の初回に判定する**（2026-09-25）
...
- **2026-09-24**: Pi の公式ドキュメント 39 ファイルに **MCP の記述が 1 件も無い**
  ...
  （静的な読みのみ。実機未確認）
- **2026-09-25**: 段 2 に着手。Pi と `omp` をしばらく常用する。あわせて
  **段 2.5（設定の回収）を計画へ追加**した。「丸ごと管理しない」という既存の
  決定があるので、回収時に何を取り何を取らないかを先に決めておく
