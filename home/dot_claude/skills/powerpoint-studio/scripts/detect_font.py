...
- Flags include_missing/include_substituted control which categories are emitted \
(default True/True).
"""
...
def configure_local_temp() -> None:
    if os.environ.get("TMPDIR"):
        return
    local_tmp = os.path.abspath(os.path.join(os.getcwd(), ".tmp", "powerpoint-studio"))
...
def parse_font_family_base_and_styles(name_norm: str) -> tuple[str, set[str]]:
    ...
    while tokens:
        ...
        tokens = tokens[:-2] if " " in matched else tokens[:-1]
...
def detect_missing_fonts_odp(pptx_path: str) -> tuple[set[str], dict[int, list[str]]]:
    ...
    with (
        tempfile.TemporaryDirectory(prefix="soffice_profile_") as prof,
        tempfile.TemporaryDirectory(prefix="soffice_convert_") as out,
    ):
...
def main() -> None:
    configure_local_temp()
    parser = argparse.ArgumentParser(
        description=(
            "Detect missing/substituted fonts for a PPTX by converting to ODP "
            "and inspecting resolved families."
        )
    )
