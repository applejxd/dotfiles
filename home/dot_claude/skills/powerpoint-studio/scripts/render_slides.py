...
def configure_local_temp() -> None:
    if os.environ.get("TMPDIR"):
        return
    local_tmp = abspath(join(os.getcwd(), ".tmp", "powerpoint-studio"))
    makedirs(local_tmp, exist_ok=True)
...
def rasterize(
    ...
    with (
        ...
        if not pdf_path or not exists(pdf_path):
            raise RuntimeError("Failed to produce PDF for rasterization (direct and ODP fallback).")
