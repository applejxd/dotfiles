...
| shell 出力の伏字化（内容の形 9 規則 + パス参照） | 配備済み。実地で確認 |
...
実装中に、`~/` 始まりの glob が**一度も当たらない**穴が見つかった。結果には
展開済みの絶対パスしか載らないためで、2 本に展開して直した。

伏字化も実地で確認した。`GITHUB_TOKEN=…` と `AKIA…` が `[伏字:…]` に替わり、
`user=demo` は無傷。`wc -l ~/.claude.json` は出力全体が伏字になった。

配備後に実履歴 1,031 呼び出しで測り直した
...
**確認はほとんど減っていない。** 減らすのが目的ではないので想定どおりで、
得たものは「`deny` の効かない経路から効く経路へ 10.5% を移したこと」。

内容の形による判定とパス判定は**両方要る**ことも分かった。
`tr '\0' '\n' < /proc/…/environ` は保護 glob に載らないので内容の形でしか
捕まらず、逆に値が不透明な秘密ファイルはパス判定でしか捕まらない。

### 残っていること

段階 2 は完了。段階 3（`verify` ツール）以降。

### 段階 2 で踏んだ誤爆

配備直後に 1 件。コミットメッセージへ `~/.claude.json` と書いただけで
`git commit` の出力が丸ごと伏字になった。判定はコマンド文字列を見るので、
**文章として書いた名前と実際の参照を区別できない**。

`deny_path_unless` で `git` / `gh` のメッセージ指定を除外して収めた。
除外はコマンド全体に当たるため `git commit -m x && cat ~/.aws/credentials` は
素通りするが、そこは誘導の `deny` が受け持つ。この穴は
`test_the_unless_hole_is_known` で明示的に固定した。
...
| 2 | plugin 基盤 + 読み取り経路の保護 | **完了**（誘導・結果フィルタ・伏字化を配備。実地で確認） |
| 3 | `verify` ツール | 未着手 |
...
配備後の実測でこの見立てが裏づけられた。誘導後も確認が出る 901 件のうち、
**状態を変えないものは 103 件（11.4%）しかない**。残りはヒアドキュメント付きの
長いスクリプトか書き込みを含むもので、allow へ移せる余地が小さい
（[段階 2 の被覆率](../research/opencode/permission/stage2-coverage.md)）。
...
| 誘導を `bypass` にも効かせる | `cd` のような作法だけの規則は代替が必ずあり、常に守らせてよい | **誤爆したときの逃げ道が消える。** `bypass` は「秘密を読むために一時的に全部外す」用途も兼ねる | — | **見送り**（2026-09-22） | — |
...
| `guide-plugin` | （なし）→ shell 出力の伏字化 | 段階 2-C の安全網 | **適用済み** |
