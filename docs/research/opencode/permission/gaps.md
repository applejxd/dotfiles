...
同じ条件では**再現しない**。5 通りの条件で測り、すべてグローバルが勝った。
...
最後の 1 行が効いている。**グローバルが何も言っていない対象を
プロジェクト側だけで deny しても通る。** つまり「並び順で負ける」のではなく
**規則ごと捨てられている**。
...
| 影響 | 内容 |
| --- | --- |
| **外部リポジトリ** | `.opencode/opencode.json` を含むリポジトリを開いても、permission は緩められない |
| **自分のリポジトリ** | エージェントが `.opencode/opencode.json` を書いても権限は広がらない |
| **既存の deny** | `**/.opencode/opencode.json` への write deny は**保険として残す**。挙動の理由が特定できていないため |
...
> **実装後に見つかった穴（2026-09-22）**: `~/` 始まりの glob から作った
> 正規表現は**一度も当たらない**。結果に載るのは**展開済みの絶対パス**
> だけで、`~` のままの形とは一致しないため。`~/.claude.json` など 4 件が
