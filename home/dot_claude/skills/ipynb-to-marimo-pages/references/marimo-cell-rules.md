...
ノートブックのファイル名が import するライブラリ名と一致すると、
そのノートブック自身が import 先になる（`sklearn.py` の `import sklearn` は
自分自身を指す）。`matplotlib.py`、`open3d.py`、`json.py` なども同様。

原本の名前がライブラリ名と衝突する場合は、内容を表す名前へリネームし、
原本との対応表に記録する。

## 8. Markdown セル内の数式には別の制約がある

`mo.md()` は KaTeX で描画するため、`\begin{equation}` や `\[...\]` は
描画されない。ディスプレイ数式は `$$` を単独行に置く形へ統一する。
詳細は `math-rendering.md`。
