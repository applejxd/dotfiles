...
- `ocs` の既知の欠陥を直すこと（[CHG-0004](closed/0004-opencode-sandbox.md) の
  B2「資格情報の露出」/ B3）
...
| 2 | 共有ランタイムとハーネスを節で分ける | 未着手（**取りかかれる**） |
| 3 | `ocs` を汎用化し、ハーネスをプロファイルで選ぶ | 未着手（動機が弱い。再開条件は「境界へ入れたいハーネスが 2 つ目になったとき」） |
| 4 | 改名（`ocs` は OpenCode 由来の名前で、汎用化すると実態と合わない） | **未決**（改名の費用と、名前が誤解を生む害の比較がついていない） |
