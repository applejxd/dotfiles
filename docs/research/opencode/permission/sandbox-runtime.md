...
敵対的なリポジトリが `~/.ssh` を自分で許可できてしまう。
`common.toml` にプロジェクトパスをキーとして持たせる。

> ここは**ランチャーが読む宣言**の話で、OpenCode の permission とは別層。
> OpenCode 側はプロジェクト設定がグローバルに勝てないことを実測したが
> （[permission の穴 §5](gaps.md#5-プロジェクト設定とグローバル設定の優劣2026-09-23-再測)）、
> 境界の宣言はこちらで塞ぐ必要がある。
